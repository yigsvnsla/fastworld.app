...
export class ConectionsService {
  ...
  async post(path: string, data: any) {
    return new Promise<any>((resolve, reject) => {
      this.toolsService.showLoading().then(async (loading) => {
        this.httpClient
          .post(`${environment.api}/${path}`, data, {
            headers: await this.headers(),
          })
          .toPromise()
          .then((res) => {
  ...
  async auth(data: any) {
    this.toolsService.showLoading().then((alert) => {
      this.httpClient
        .post<Auth>(`${environment.api}/clients/auth`, data)
        .toPromise()
        .then(async (res) => {
          if (res.status) {
            ...
            this.cookie.set(environment.cookieTag, {
              jwt: res.jwt,
              email: res.email,
            });
            this.httpClient
              .get<any>(`${environment.api}/clients?email_eq=${res.email}`, {
                headers: await this.headers(),
              })
              .toPromise()
              .then((data) => {
                if (
                  ...
                  parseISO(data[0].memberships.expire) <= new Date(Date.now())
                ) {
                  data[0].memberships = null;
                }
  ...
  async guest(data: {
    ...
    return new Promise<any>((resolve, reject) => {
      this.toolsService.showLoading().then(async (loading) => {
        this.httpClient
          .post(`${environment.api}/products/guest`, data)
          .toPromise()
          .then((res) => {
            resolve(res);
          })
  ...
  async guestPost(path, data) {
    return <any>(
      this.httpClient.post(`${environment.api}/${path}`, data).toPromise()
    );
  }

  isOnline() {
    this.getWindowInternet().subscribe(async (net) => {
      if (net) {
  ...
  async startService({ email }) {

    let watcher = await BackgroundGeolocation.addWatcher(
      {
